    ...
    def __call__(self, results):
        ...
        if "gt_masks" in results:
            ...
            for mask in results["gt_masks"]:
                expand_mask = np.full((int(h * ratio), int(w * ratio)), 0).astype(
                    mask.dtype
                )
        ...
        if "gt_semantic_seg" in results:
            ...
            expand_gt_seg = np.full(
                (int(h * ratio), int(w * ratio)), self.seg_ignore_label
            ).astype(gt_seg.dtype)
    ...
    def __repr__(self):
        ...
        repr_str += (
            ...
            "seg_ignore_label={})".format(
                self.mean, self.to_rgb, self.ratio_range, self.seg_ignore_label
            )
        )
        return repr_str
        ...
        while True:
            ...
            for i in range(50):
                ...
                if len(overlaps) > 0:
                    ...
                    if "gt_masks" in results:
                        valid_masks = [
                            results["gt_masks"][i] for i in range(len(mask)) if mask[i]
                        ]
