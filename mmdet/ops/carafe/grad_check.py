...
mask = (
    torch.randn(2, 25, 200, 200, requires_grad=True, device="cuda:0").sigmoid().float()
)
loop_num = 500
...
print(
    "\nCARAFE time forward: {} ms/iter | time backward: {} ms/iter".format(
        (time_forward + 1e-3) * 1e3 / loop_num, (time_backward + 1e-3) * 1e3 / loop_num
    )
)
...
print(
    "\nCARAFE naive time forward: {} ms/iter | time backward: {} ms/iter".format(
        (time_naive_forward + 1e-3) * 1e3 / loop_num,
