...
class _GridSampler(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input, grad_grid = func(
            grad_output, input, grid, mode_enum, padding_mode_enum, align_corners
        )
...
def grid_sample(
    ...
    if torch.__version__ >= "1.3":
        ...
        assert input.device == grid.device, (
            ...
            "but input is on {} and grid is on {}".format(input.device, grid.device)
        )
        assert input.dtype == grid.dtype, (
            "expected input and grid to have the same dtype, "
            "but input has {} and grid has {}".format(input.dtype, grid.dtype)
        )
        ...
        assert grid.size(-1) == input.dim() - 2, (
            ...
            "but got grid with sizes ".format(input.dim() - 2, grid.size())
        )
