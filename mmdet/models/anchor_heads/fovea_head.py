    ...
    def get_bboxes_single(
        ...
        for cls_score, bbox_pred, featmap_size, stride, base_len, (y, x) in zip(
            ...
            if (nms_pre > 0) and (scores.shape[0] > nms_pre):
                ...
                y = y[topk_inds]
                x = x[topk_inds]
            x1 = (stride * x - base_len * bbox_pred[:, 0]).clamp(
                min=0, max=img_shape[1] - 1
            )
            y1 = (stride * y - base_len * bbox_pred[:, 1]).clamp(
                min=0, max=img_shape[0] - 1
            )
            x2 = (stride * x + base_len * bbox_pred[:, 2]).clamp(
                min=0, max=img_shape[1] - 1
            )
            y2 = (stride * y + base_len * bbox_pred[:, 3]).clamp(
                min=0, max=img_shape[0] - 1
            )
