...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        ...
        proposal_list = (
            self.simple_test_rpn(x, img_metas, self.test_cfg.rpn)
        ...
        for i in range(self.num_stages):
            ...
            if self.use_TSD[i]:
                _, _, cls_score, bbox_pred, delta_c, delta_r = bbox_head(
                    bbox_feats, x[: len(bbox_roi_extractor.featmap_strides)], rois
                )
            else:
                cls_score, bbox_pred = bbox_head(bbox_feats)
            ...
            if i < self.num_stages - 1:
                ...
                if self.use_TSD[i]:
                    ...
                    rois = bbox_head.regress_by_class(
                        rois_r, bbox_label, bbox_pred, img_metas[0]
                    )
                else:
                    rois = bbox_head.regress_by_class(
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                merged_masks = merge_aug_masks(
                    aug_masks, [img_metas] * self.num_stages, self.test_cfg.rcnn
                )
    ...
    def aug_test(self, imgs, img_metas, proposals=None, rescale=False):
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                merged_masks = merge_aug_masks(
                    aug_masks, aug_img_metas, self.test_cfg.rcnn
                )
